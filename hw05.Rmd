...
```{r}
education_orig = read_csv("D:/uwm/course/stat240/data/education.csv")
education_orig
```
...
ggplot(education_orig,aes(x=pct_f_bach,y=pct_m_bach))+
  ...
  xlab("percentage of female")+
  ylab("percentage of male")+
  ggtitle("Having a Bachelor's Degree",subtitle = "age >= 25")
```
...
> The percentage of males and females having at least a bachelor's degree is roughly the same when the zip code is consistent, and is concentrated between 10% and 40%. 

###  2c
What do you think might be true about the zip codes with strong outliers, for example, where percentages are 0% or 100%?

> RESPOND HERE
  It is possible that the data obtained from this area is limited and not representative.
...
```{r}
##Read in the obesity data set from the file *obesity.csv*
prob4 = read_csv("D:/uwm/course/stat240/data/obesity.csv")%>%
  mutate(zip = as.character(zip)) %>%
  mutate(obese_pop = pop * (obese/n),
         non_obese_pop = pop * (1 - obese/n))
prob4

##Create a summary of the data named `adult_pop_sum`
adult_pop_sum = prob4 %>%
  filter(age!="05-17")%>%
  group_by(zip,sex) %>%
...
##Display this data with side-by-side box-plots
ggplot(adult_pop_sum,aes(x=total_pop,y=sex))+
  geom_boxplot()+
  xlab("total population")+
  ylab("sex")+
  ggtitle("Total Population For Each Zip Code ",subtitle="age 18 and older")
```
...
prob5 %>%
  filter(pct_bachelors==100.0 | pct_bachelors==0.0)%>%
...
> The zip codes with the highest and lowest total population are the only two zip codes with 100% bachelor degree in the tibble. 
...
prob5 %>%
  drop_na() %>%
  mutate(pop_with_bach=pct_bachelors*total_pop/100) %>%
  group_by(sex) %>%
  summarise(total_pop=sum(total_pop),total_bach=sum(pop_with_bach))%>%
  mutate(pct_bach=total_bach/total_pop)%>%
  select(sex,pct_bach)%>%
  mutate(pct_bach = paste0(round(pct_bach * 100, 2), "%"))
```

> The percentages of women and men ages 25 and older in Wisconsin with a bachelors degree are 29.62% and 27.27%.
...
```{r}
adult_obesity_sum = read.csv("D:/uwm/course/stat240/data/obesity.csv")%>%
  filter(age!="05-17")%>%
  drop_na()%>%
  group_by(zip)%>%
  mutate(remain_rows=n())%>%
  ungroup()%>%
  filter(remain_rows>=8)%>%
  mutate(obese_pop=obese*pop/n)%>%
  group_by(zip,sex)%>%
...
prob8 = adult_obesity_sum %>%
  ...
  mutate(pct_obese=100*obese_pop/total_pop)
prob8

ggplot(prob8,aes(x=pct_bachelors,y=pct_obese,col=sex))+
  geom_point()+
  geom_smooth(se = FALSE)+
  xlab("education percentage")+
  ylab("obesity percentage")+
  ggtitle("Obesity Versus Education")
```

Describe the pattern you see:
how does obesity rate change among men and women as education percentage increases?

> When the education percentage goes down, obesity percentage also goes down.Women's rate have a steeper change than men's 
