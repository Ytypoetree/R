...
p1_stat = p1%>%
  mutate(xp=x*p)%>%
  summarise(mean=sum(xp),variance=sum((x-mean)^2*p),standard_deviation=sqrt(variance))
p1_stat
```
...
```{r}
mu=p1_stat$mean
sigma=p1_stat$standard_deviation
ggplot(p1,aes(x=x,y=p))+
  geom_segment(aes(xend = x, yend = 0), color = "blue", size=2) +
  geom_vline(xintercept = mu, color = "red", linetype = "dashed") +
  geom_vline(xintercept = mu + c(-1,1)*sigma,color = "black",linetype = "dotted")+
  geom_vline(xintercept = mu + c(-2,2)*sigma,color = "black",linetype = "dotted")+
  ylab("P(X=x)") +
  ggtitle("Distribution of p1")
```
...
q5=p1%>%
  select(x,cdf)%>%
  add_row(x = c(-1,9), cdf = c(0,1))%>%
  arrange(x)
q5
  
ggplot(q5, aes(x = x, y = cdf)) +
  geom_step() +
  labs(title = "Cumulative Distribution Function for p1",
       x = "x",
       y = "F(x)")
```
...
> X is a discrete random variable. X cannot take values from 2 to 3 so F(x)=0.24 for all X from 2 to 3 and F(x)=0.25 only when x=3.
...
> 2, the same as the median value of 'qbinom(0.5,5,0.4'
...
> 0.80 is smaller than pnorm(60,50,10) so the 0.80 quantile of this distribution is smaller than 60.
